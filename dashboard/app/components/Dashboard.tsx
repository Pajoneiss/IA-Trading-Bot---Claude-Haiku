...
// PnL Cards
const PnLCards = ({ pnl }: { pnl: PnLSummary | null }) => {
  const cards = [
    { label: '🏆 ALL TIME', pct: pnl?.pnl_all_time_pct || 0, usd: pnl?.pnl_all_time_usd || 0, highlight: true },
    { label: '📅 24H', pct: pnl?.pnl_day_pct || 0, usd: pnl?.pnl_day_usd || 0 },
  ...
  return (
    <div className="grid grid-cols-2 md:grid-cols-4 gap-2 mb-3">
      {cards.map((c, i) => (
        <Card key={i} className={`p-3 ${c.highlight ? (c.pct >= 0 ? 'border-[#00ff88]/30 bg-[#00ff88]/5' : 'border-[#ff4d4d]/30 bg-[#ff4d4d]/5') : ''}`}>
          <div className="text-xs text-gray-500">{c.label}</div>
          <div className={`text-2xl font-bold ${c.pct >= 0 ? 'text-[#00ff88]' : 'text-[#ff4d4d]'}`}>{c.pct >= 0 ? '+' : ''}{c.pct.toFixed(2)}%</div>
          <div className={`text-sm ${c.pct >= 0 ? 'text-[#00ff88]' : 'text-[#ff4d4d]'}`}>{c.pct >= 0 ? '+' : ''}${c.usd.toFixed(2)}</div>
        </Card>
      ))}
    </div>
  )
